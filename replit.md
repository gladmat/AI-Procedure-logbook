...
- **JWT Token Revocation**: Users table includes `tokenVersion` field. JWT tokens include `tokenVersion` in payload, validated against current user version on each authenticated request. Password changes automatically increment `tokenVersion`, immediately revoking all existing tokens.
...
- **Rate Limiting**: Auth endpoints protected against brute force attacks. Express `trust proxy` enabled for accurate client IP detection behind reverse proxies.
- **Seed Endpoint Protection**: Database seeding endpoint protected with SEED_TOKEN header validation in production.
- **Patient Identifier Privacy**: Patient identifiers in local case index are SHA-256 hashed using expo-crypto, preventing plaintext storage even in local storage. Migration automatically upgrades existing data.
