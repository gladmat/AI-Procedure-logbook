...
import type { DiagnosisPicklistEntry, StagingSelections, EvaluatedSuggestion } from "@/types/diagnosis";
      ...
      {defaults.length > 0 ? (
        <View style={styles.chipGroup}>
          {defaults.map((suggestion) => {
            const isSelected = selectedProcedureIds.has(suggestion.procedurePicklistId);
            return (
              <SuggestionChip
...
function SuggestionChip({
  ...
  return (
    <View>
      ...
      {suggestion.isConditional && !isActive && suggestion.conditionDescription ? (
        <ThemedText
          type="small"
