...
export const AO_HAND_CLASSIFICATION: AOHandClassification = {
  ...
  bones: [
    ...
    },
    {
      ...
      segments: { "1": "Proximal end (base)", "2": "Diaphysis (shaft)", "3": "Distal end (head)" },
      typeRulesBySegment: {
        "1": {
...
export function generateAOCode(params: {
  ...
  const { familyCode, type, subBoneId, finger, phalanx, segment, qualifications } = params;
  ...
  if (["71", "72", "73", "74", "75"].includes(familyCode)) {
    ...
    if (familyCode === "72" && (type === "B" || type === "C") && qualifications && qualifications.length > 0) {
...
export function validateAOCode(code: string): { valid: boolean; error?: string } {
...
export function parseAOCode(code: string): {
  ...
  if (carpalMatch) {
    const boneNames: Record<string, string> = {
      "71": "Lunate", "72": "Scaphoid", "73": "Capitate", "74": "Hamate", "75": "Trapezium"
    };
    return {
      familyCode: carpalMatch[1],
      boneName: boneNames[carpalMatch[1]],
      type: carpalMatch[2],
  ...
  if (otherCarpalMatch) {
    const subBoneNames: Record<string, string> = { "1": "Pisiform", "2": "Triquetrum", "3": "Trapezoid" };
    return {
      familyCode: "76",
      boneName: subBoneNames[otherCarpalMatch[1]],
  ...
  if (metacarpalMatch) {
    return {
      familyCode: "77",
      boneName: `${FINGER_NAMES[metacarpalMatch[1]]} Metacarpal`,
  ...
  if (phalanxMatch) {
    return {
      familyCode: "78",
      boneName: `${FINGER_NAMES[phalanxMatch[1]]} ${PHALANX_NAMES[phalanxMatch[2]]} Phalanx`,
