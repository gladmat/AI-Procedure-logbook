...
export function FreeFlapClinicalFields({
  ...
  // Flaps where a skin island decision is clinically meaningful
  ...
  const handleFlapTypeChange = (flap: FreeFlap) => {
    ...
    onUpdate({
      ...
      // Reset skin island when flap changes
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Skin island toggle — only for muscle flaps where it's a real decision */}
      {showSkinIsland ? (
        <SelectField
          label="Skin Island"
...
interface ProcedureClinicalDetailsProps {
  specialty: Specialty;
  procedureType: string;
  picklistEntryId?: string;         // preferred — drives hasFreeFlap cleanly
...
export function ProcedureClinicalDetails({
  ...
  // Prefer picklist-driven detection — reliable, no string matching
  const picklistEntry = picklistEntryId ? findPicklistEntry(picklistEntryId) : undefined;
  const isFreeFlapProcedure =
    picklistEntry?.hasFreeFlap ??
    // Legacy fallback for procedures logged before picklist adoption
