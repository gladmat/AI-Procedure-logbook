...
export function ProcedureEntryCard({
  ...
  // Called when surgeon selects from the new hierarchical picklist
  const handlePicklistSelect = (entry: ProcedurePicklistEntry) => {
    onUpdate({
      ...
      // Clear clinical details when procedure changes to avoid stale data
      clinicalDetails: undefined,
    });
  };

  // Fallback: called for specialties still using legacy flat PROCEDURE_TYPES
