...
export type AnatomicalRegion = 
  ...
  | "upper_arm" 
  | "head_neck"
  | "breast_chest";  // internal mammary / thoracodorsal recipient vessels
...
export type FreeFlap = 
  ...
  | "latissimus_dorsi" 
  | "gracilis"
  | "tug"              // Transverse Upper Gracilis — gracilis variant
...
/**
 * Maps FreeFlap enum to canonical SNOMED CT concept.
 * These codes drive the stored snomedCtCode on FreeFlapDetails.
 * Entries marked VERIFY should be confirmed in https://browser.ihtsdotools.org/
 */
export const FLAP_SNOMED_MAP: Partial<Record<FreeFlap, { code: string; display: string }>> = {
  ...
  tug:               { code: "234297004", display: "Free gracilis flap (procedure)" }, // TUG variant — same parent
  scip:              { code: "234299000", display: "Free superficial circumflex iliac artery flap (procedure)" }, // VERIFY
...
/**
 * Maps AnatomicalRegion to SNOMED CT body structure concept.
 * Used when building post-coordinated procedure expressions.
 * Entries marked VERIFY should be confirmed in the SNOMED browser.
 */
export const RECIPIENT_SITE_SNOMED_MAP: Partial<Record<AnatomicalRegion, { code: string; display: string }>> = {
  lower_leg:    { code: "30021000",  display: "Lower leg structure (body structure)" },  // VERIFY
  knee:         { code: "72696002",  display: "Knee region structure (body structure)" }, // VERIFY
  foot:         { code: "56459004",  display: "Foot structure (body structure)" },
  thigh:        { code: "68367000",  display: "Thigh structure (body structure)" },       // VERIFY
...
export interface FreeFlapDetails {
  ...
  flapType?: FreeFlap;
  // SNOMED CT — set automatically from FLAP_SNOMED_MAP when flapType is selected
  flapSnomedCode?: string;
  flapSnomedDisplay?: string;
  flapCommonName?: string;
  composition?: string;
  harvestTechnique?: string;
  skinIsland?: boolean;           // relevant for Gracilis, TUG, Serratus — muscle ± skin paddle
  recipientSite?: string;
  recipientSiteRegion?: AnatomicalRegion;
  // SNOMED CT — set automatically from RECIPIENT_SITE_SNOMED_MAP when recipientSiteRegion selected
...
export interface CaseProcedure {
  ...
  specialty?: Specialty;
  subcategory?: string;           // e.g. "Free Flap Coverage" — from picklist entry
  picklistEntryId?: string;       // stable id from ProcedurePicklistEntry — drives SNOMED, hasFreeFlap
  tags?: ProcedureTag[];
  snomedCtCode?: string;
  snomedCtDisplay?: string;
  localCode?: string;
